...
public class RaspRC522 {
    ...
    private final int MAX_LEN = 16; //セクタバイト数
...
//        Gpio.wiringPiSetup();           //Enable wiringPi pin schema
     ...
     * @param data
     * @param dataLen   16以下
     * @param back_data 返ってきたデータ
     * @param back_bits 返ってきたビット数
     * @param backLen   返ってきたバイト数
     * @return
     */
     ...
     *
     * @param back_data 5バイト 4バイトタグID+1バイト☑
     * @return
     */
    public int AntiColl(byte[] back_data) {
        int status;
        byte[] serial_number = new byte[2];   //2バイト命令
